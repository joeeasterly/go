...
def print_storage(record_dict):
    ...
    max_label_length = 32  # Max characters for label on the first line
    first_line_label, overflow_label = split_label(label, max_label_length)
    print("Storage Record: " + first_line_label)
    if overflow_label:
        print(overflow_label.rjust(50))  # Right-align the overflow text
    # Handle description wrapping
    max_desc_length = 32  # Max characters for description on the first line
    first_line_desc, overflow_desc = split_label(description, max_desc_length)
    print("Description: " + first_line_desc)
    if overflow_desc:
        print(overflow_desc.rjust(50))  # Right-align the overflow text
    len_location = len("SHCN: " + location)
    len_notion = len("Notion : " + notion_id)
    loc_notion_spacing = 50 - (len_location + len_notion)
    len_last_updated = len("Last Updated: " + last_updated)
