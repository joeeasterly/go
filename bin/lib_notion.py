...
def link_notion_media(notion_id, mungo_id, mungo_label):
    # Get the NOTION_API_KEY from the environment
    NOTION_API_KEY = os.getenv('NOTION_API_KEY')
    ...
    # Hard-coded DATABASE_ID
    DATABASE_ID = 'cd3af81e5c574216af404f4b0643dffd'
    ...
    if response.status_code == 200:
        ...
        if data.get('results'):
            ...
            if update_response.status_code == 200:
                return green("Notion record updated successfully: https://notion.so/inv-" + notion_id)
            else:
                return "Failed to update Notion record."
        else:
            return "Notion record not found."
